...
@jmub.on(events.NewMessage(outgoing=True, pattern=r"\.بوت المليار"))
async def _(event):
    if ispay[0] == "yes":
...
@jmub.on(events.NewMessage(outgoing=True, pattern=r"\.جمع"))
async def _(event):
    if ispay[0] == "yes":
        ...
        for i in range(100):
            ...
            if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                await jmub.send_message(event.chat_id, f"مافي قنوات بلبوت")
                break
            ...
            try:
                ...
                chs += 1
                await jmub.send_message(event.chat_id, f"تم الاشتراك في {chs} قناة")
            except:
